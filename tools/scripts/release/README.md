...
Indicating username ensures we only watch workflows that are created by the executor. The release script also allows for modifications provided by the `bump` and `publish` scripts, for example to create a release that uses a different `RELEASE_VERSION` different from the default (minor)

```bash
RELEASE_VERSION=major GITHUB_USERNAME=metaclips release.sh
```

We can skip steps during a release by defining as `true`
